...
{
    return (*((const u8*)buffer + (bit_offset >> 3)) & (1 << (bit_offset & 7)));
}
...
static remap_segment_ctx_t *save_remap_init_segments(remap_header_t *header, remap_entry_ctx_t *map_entries, u32 num_map_entries)
{
    ...
    for(i = 0; i < header->map_segment_count; i++)
    {
        ...
        while(entry_idx < num_map_entries && map_entries[entry_idx - 1].virtual_offset_end == map_entries[entry_idx].virtual_offset)
        {
            ...
            remap_entry_ctx_t **ptr = calloc(seg->entry_count + 1, sizeof(remap_entry_ctx_t*));
            if (!ptr)
            {
                LOG_MSG_ERROR("Failed to allocate memory for remap segment entry #%u!", entry_idx);
                goto end;
...
static bool save_ivfc_storage_init(hierarchical_integrity_verification_storage_ctx_t *ctx, u64 master_hash_offset, ivfc_save_hdr_t *ivfc)
{
    ...
    ctx->level_validities = calloc((ivfc->num_levels - 1), sizeof(validity_t*));
    ...
    for(u32 i = 1; i < ivfc->num_levels; i++)
    {
        ...
        level_data->block_validities = calloc(level_data->sector_count, sizeof(validity_t));
...
bool save_hierarchical_directory_table_get_file_entry_by_path(hierarchical_save_file_table_ctx_t *ctx, const char *path, save_fs_list_entry_t *entry)
{
    ...
    u32 index = save_fs_list_get_index_from_key(&ctx->directory_table, &key, NULL);
    if (index == 0xFFFFFFFF)
    {
        LOG_MSG_ERROR("Unable to get table index for file \"%s\"!", path);
        return false;
    ...
    if (!save_fs_list_get_value(&ctx->directory_table, index, entry))
    {
        LOG_MSG_ERROR("Unable to get file entry for \"%s\" from index!", path);
        return false;
    }
...
bool save_process(save_ctx_t *ctx)
{
    ...
    ctx->data_remap_storage.map_entries = calloc(ctx->data_remap_storage.header->map_entry_count, sizeof(remap_entry_ctx_t));
    ...
    ctx->meta_remap_storage.map_entries = calloc(ctx->meta_remap_storage.header->map_entry_count, sizeof(remap_entry_ctx_t));
    ...
    ctx->journal_storage.map.entries = calloc(ctx->journal_storage.map.header->main_data_block_count, sizeof(journal_map_entry_t));
