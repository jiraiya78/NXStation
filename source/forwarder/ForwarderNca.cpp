...
#include "forwarder/HblEmbed.hpp"
...
struct ByteView {
    ...
    ByteView(const u8* dataIn, u64 sizeIn) : data(dataIn), size(sizeIn) {}
    ByteView(const std::vector<u8>& bytes) : data(bytes.data()), size(bytes.size()) {}
    ByteView(const u8* keyBytes) : data(keyBytes), size(0x20) {}
};
...
struct ForwarderProgress {
    void SetTitle(const std::string&) {}
    void SetImageDataConst(const std::vector<u8>&) {}
    struct Transfer {
        void UpdateTransfer(int, int) {}
    };
    Transfer NewTransfer(const char*) { return {}; }
};
...
struct BufHelper {
    BufHelper() = default;
    BufHelper(ByteView data) { write(data); }
...
void add_file_entry(FileEntries& entries, const char* name, const std::vector<u8>& data) {
    add_file_entry(entries, name, data.data(), data.size());
}

auto build_ivfc_master_hash(const std::vector<u8>& level1) -> std::vector<u8> {
...
u64 forwarderTitleHashBits(const std::string& nro_path, const std::string& args) {
    const std::string full_args = args.empty() ? nro_path : (nro_path + ' ' + args);
    const std::string hash_path = nro_path + full_args;

    u64 hash_data[SHA256_HASH_SIZE / sizeof(u64)];
    sha256CalculateHash(hash_data, hash_path.data(), hash_path.length());
    return hash_data[0] & 0x00FFFFFFFFFFF000;
}

} // namespace

u64 forwarderTitleId(const std::string& nro_path, const std::string& args) {
    return 0x0500000000000000 | forwarderTitleHashBits(nro_path, args);
}

auto install_forwarder_internal(ForwarderProgress* pbox, sf::ForwarderConfig& config,
                                NcmStorageId storage_id) -> Result {
    if (pbox) {
    ...
    // tid is hashed from the pre-rewrite args, so derive it before fixing them up
    const u64 hash_bits = forwarderTitleHashBits(config.nro_path, config.args);
    ...
    const u64 old_tid = 0x0100000000000000 | hash_bits;
    const u64 tid = 0x0500000000000000 | hash_bits;
    ...
    if (config.program_nca.empty()) {
        ...
        FileEntries exefs;
        add_file_entry(exefs, "main", sf::forwarder::kHblMain, sf::forwarder::kHblMain_size);
        add_file_entry(exefs, "main.npdm", sf::forwarder::kHblNpdm, sf::forwarder::kHblNpdm_size);
    ...
    {
        if (pbox)
            pbox->NewTransfer("Updating ncm database").UpdateTransfer(4, 8);
    ...
    // push record
    {
        ...
        if (R_FAILED(rc) && rc != 0x410) { // not found
            SF_LOG_W("Forwarder", "Could not remove old forwarder title (0x%x)",
...
u64 forwarderTitleId(const std::string& nro_path, const std::string& args)
{
    return sphaira::forwarderTitleId(nro_path, args);
}

Result installForwarderNca(ForwarderConfig& config, NcmStorageId storage_id)
{
    sphaira::ForwarderProgress progress;
    return sphaira::install_forwarder_internal(&progress, config, storage_id);
}
